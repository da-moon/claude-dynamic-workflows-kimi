...
import { versionDriftNote, VERIFIED_KIMI_VERSION } from "../src/kimiVersion.js";
...
{
  const have = ["kimi-latest", "kimi-k2-6", "kimi-k2-5", "kimi-k2", "kimi-lite"];
  assert.equal(resolveModel("claude-opus-4-8", have), "kimi-latest", "opus -> latest Kimi");
  assert.equal(resolveModel("sonnet", have), "kimi-latest", "sonnet -> latest Kimi");
  assert.equal(resolveModel("haiku", have), "kimi-lite", "haiku -> efficient Kimi");
  assert.equal(resolveModel("kimi", have), "kimi-latest", "kimi alias -> explicit id");
  assert.equal(resolveModel("kimi-k2-6", have), "kimi-k2-6", "available id passes through");
...
// 9) agentType: read system prompt + model from .kimi/agents/<name>.md, with .claude fallback.
{
  ...
  await mkdir(join(root, ".kimi", "agents"), { recursive: true });
  await writeFile(
    join(root, ".kimi", "agents", "terse.md"),
...
// 10) retry classification: transient -> retry; permanent -> no retry.
{
  const transientCode = Object.assign(new Error("upstream blip"), {
    kimiErrorInfo: "ResponseStreamDisconnected",
  });
  const httpObj = Object.assign(new Error("boom"), {
    kimiErrorInfo: { HttpConnectionFailed: { httpStatusCode: 503 } },
  });
  ...
  assert.equal(
    isRetryable(Object.assign(new Error("x"), { kimiErrorInfo: "ContextWindowExceeded" })),
    false,
...
{
  const models = [
    { id: "kimi-latest", isDefault: true },
    { id: "kimi-k2-6", isDefault: false },
    { id: "kimi-k2-5", isDefault: false },
    { id: "kimi-k2", isDefault: false },
    { id: "kimi-lite" },
    { id: "kimi-mini" },
    { id: "kimi-spark" },
  ];
  assert.equal(pickFrontier(models), "kimi-latest", "picks the latest flagship tier");
  assert.equal(
    pickFrontier(["kimi-k2-5", "kimi-k2-6"]),
    "kimi-k2-6",
    "prefers k2-6 over k2-5 when latest is unavailable",
  );
  assert.equal(
    pickFrontier(["kimi-k2", "kimi-mini"]),
    "kimi-k2",
    "string ids: strength-max, skips mini",
  );
...
// 13) effort precedence: pin > per-call > auto > --effort flag > omitted.
{
  ...
  assert.equal(r4, "(none)", "no effort anywhere -> omitted (Kimi config default)");
}
...
// 32) --plan handles start/wait/steer/waitAny WITHOUT calling the session driver.
{
  ...
  assert.equal(called, false, "--plan never calls the session driver (no Kimi, no tokens)");
