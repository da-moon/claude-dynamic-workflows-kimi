...
//
// Nothing here mentions Claude or Kimi: this is the scheduling glue that the
// Workflow tool description specifies. Only agent() reaches a model, via the
// kimiAgent seam. Concurrency is capped exactly like the native runtime:
...
import { startKimiSession } from "./kimiSession.js";
...
export function createRuntime({
  ...
  journal = null,
  runAgent = kimiAgent, // seam: injected in tests to capture resolved opts
  startSession = startKimiSession, // seam: injected in tests for sessionful workers
  ...
  // The workflow-facing handle around a live KimiSessionDriver. Owns per-turn
  ...
  // ── Plan mode (--plan): no Kimi. Sessions become deterministic skeletons so the
