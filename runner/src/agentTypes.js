...
// reading the subagent markdown definition from .kimi/agents/<name>.md (preferred)
// or .claude/agents/<name>.md (fallback). Project scope (walking up from cwd for
// the agents dir) takes precedence over the user scope (~/.kimi or ~/.claude).
...
export async function loadAgentType(name, cwd = process.cwd()) {
  ...
  const kimiRel = join(".kimi", "agents", `${name}.md`);
  const claudeRel = join(".claude", "agents", `${name}.md`);
  const found =
    (await findUp(cwd, kimiRel)) ??
    (await tryRead(join(homedir(), ".kimi", "agents", `${name}.md`))) ??
    (await findUp(cwd, claudeRel)) ??
